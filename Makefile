...
IV = $(IV1) $(IV2) $(IV3) $(EMP)
uSWIFT_RUNTIME_DIR ?= $(SCRIPTDIR)/uSwift-$(ARCH)$(SUBARCH_DIR)
SWIFT_SHIMS_DIR ?= $(SCRIPTDIR)/uSwiftShims
# some day libSwiftExperimentalRuntime should be folded into libSwift
LINK_LIBRARIES := $(ADDED_CORE_LINK) -L "$(uSWIFT_RUNTIME_DIR)" -lSwiftExperimentalRuntime -lSwift
...
SWIFTC_OPTS := -no-link-objc-runtime -Xfrontend -disable-reflection-metadata\
 ...
 $(SWIFT_INCLUDE_DIRS) $(ARCH_INCLUDES) -Xfrontend -disable-implicit-concurrency-module-import \
...
CLANG_OPTS ?= -fmodules -std=c99 -O3 $(C_PACKAGES_OPTS) $(CLANG_INCLUDE_DIRS) $(ARCH_DEFINES) $(TARGET_OPTS) $(ARCH_CLANG_OPTS) $(MODULE_INCLUDE_DIRECTORIES)
CLANG_PP_OPTS ?= -fmodules -fcxx-modules -std=c++11 -fno-rtti -O3 $(C_PACKAGES_OPTS) $(CLANG_INCLUDE_DIRS) $(ARCH_DEFINES) $(TARGET_OPTS) \
...
$(ALL_ASM_OBJECTS) : $(ALL_ASM_FILES_ND)
	$(CLANG) -c -o $@ $(notdir $(<:%.o=%.s))
